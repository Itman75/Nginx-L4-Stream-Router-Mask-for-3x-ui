...
* **Гибкая установка:** Интерактивный выбор версии панели управления `3x-ui` (последний стабильный релиз или конкретный тег версии, например, `v2.9.4`). 
  > **Важно:** При использовании L4-маршрутизации панель должна функционировать без собственных SSL-сертификатов. При установке панели в интерактивном меню рекомендуется выбрать пункт 4 и отказаться от автоматического выпуска сертификатов (ответить `N`), так как трафик будет проксироваться через локальный интерфейс.
...
./secure-vps.sh
```
> **Результат выполнения:** Опциональное обновление ОС, установка утилит администрирования, активация BBR, отключение IPv6, создание безопасного sudo-пользователя, импорт или генерация SSH-ключей Ed25519, изменение porta SSH, отключение парольного входа, базовая настройка UFW, Fail2ban и установка панели 3X-UI (без локальных SSL-сертификатов, с проксированием через localhost).
...
> Внутри панели 3X-UI **пути к файлам SSL-сертификатов везде должны оставаться пустыми**. Внешнее TLS-шифрование для веб-интерфейса, подписок и сайтов полностью обеспечивается на стороне Nginx.
...
## 🔒 Таблица портов и правил фильтрации (UFW) (Таблиа примерная. Порты могут выбираться Вами при развёртывании)
...
| `443/TCP` | Входящий | **Разрешен** | Единая точка входа Nginx Stream (Сайт, Панель, Подписки, VLESS) |
