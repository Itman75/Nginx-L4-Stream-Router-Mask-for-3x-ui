...
* **Автоматизация SSL Let's Encrypt:** Официальный Certbot выпускает сертификат для самой панели (Primary) и для SNI инбаундов (Alternative, каждый домен привязывается к определяемому вами порту при развёртывании). Реализован deploy-hook для мгновенной перезагрузки Nginx при автопродлении сертификата.
