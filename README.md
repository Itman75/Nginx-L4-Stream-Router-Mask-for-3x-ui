...
Набор скриптов для автоматизированного развёртывания защищённой инфраструктуры прокси-серверов на чистых ОС **Ubuntu (20.04 / 22.04 / 24.04)** и **Debian (11 / 12)**. 

Проект объединяет скрипт глубокого укрепления безопасности ОС (`secure-vps.sh`) и официальный L4 Stream-маршрутизатор Nginx (`setup_mask.sh` v4.1.0) с интеграцией панели управления **3X-UI**, защитой от активного сетевого сканирования (Active Probing) со стороны систем DPI и полной поддержкой транспортов VLESS REALITY, xHTTP и Hysteria 2.
...
Ваш файл **`README.md`** полностью собран и оформлен! Вы можете просто скопировать этот код и вставить его в файл репозитория на GitHub. Наслаждайтесь результатом вашей прекрасной работы!
