...
<p align="center">
  <b>Комплексный монолитный автоустановщик высокопроизводительной, отказоустойчивой и скрытной прокси-инфраструктуры.</b><br>
...
* **Двухрежимный установщик (Dual-Mode Engine):**
  * **Режим 1: Чистая установка (Clean Install)** — развертывание с нуля для новых VPS с автоматической генерацией учетных записей, UUID и подписок.
  * **Режим 2: Безопасное обновление (Safe Migration Mode)** — интеллектуальное обновление серверов v6.x со **100% сохранением всех существующих клиентов, их UUID, ключей, паролей, статистики и хостов** без обрыва связи у пользователей.
* **Штатное ядро Xray-core v26.7.28 из коробки:** Полный отказ от внешних скачиваний архивов ядра — панель 3X-UI v3.8.5 запускает вшитый нативный бинарник `xray` с мгновенным статусом `● Xray · Запущен v26.7.28`.
...
* **Steal-Oneself REALITY с защитой Anti-Loop (Port 9443):** Камуфляж под собственный поддомен `cdn.`. Обычные веб-браузеры и сканеры активного зондирования перенаправляются Xray на локальный слушатель `127.0.0.1:9443` с Proxy Protocol (`xver: 1`), полностью исключая петлю бесконечной пересылки пакетов.
* **Classic External REALITY:** Маскировка под внешние ресурсы (Т-Банк, Samsung, Apple) с прямым сбросом неавторизованных запросов (`xver: 0`).
...
* **Автоматизация сгруппированных хостов (Host Groups):** Автоматическое заполнение таблицы `hosts` в 3X-UI с поддержкой схемы GORM:
...
* **Приватный DoH AdGuard Home:** Интегрированный DNS-over-HTTPS резолвер на поддомене `dns.` со Split-DNS и авторизацией по токенам `ClientID` для защиты от несанкционированного сканирования (Open Resolver).
...
* **Профиль [1] RU:** Резолверы Яндекс DNS (`77.88.8.8`, `77.88.8.1`), пулы зеркал GitHub (`ghfast.top`, `ghproxy.net`), принудительный DoH over TCP.
...
* Автоматическая установка последней версии панели 3X-UI со встроенным ядром Xray-core v26.7.28.
* Нативная генерация постквантового ключа **ML-KEM-768** через команду `$XRAY_BIN vlessenc`.
* **Direct SQLite Injection:** Внедрение параметров инбаундов и настроек роутинга напрямую в `/etc/x-ui/x-ui.db`. В режиме Safe Migration сохраняются все действующие клиенты и ключи.
* **Автоматическое создание GORM Host Groups:** Заполнение таблицы `hosts` с автоматической генерацией групп `ALL_443` и `xHTTP`.
...
* **Домены:** Привязаны к IP сервера минимум **1 домен** (`main.primary.domain` — главный для Nginx, веб-панели и сайта-маскировки) и **2 субдомена** (`cdn.main.primary.domain` для REALITY Steal-Oneself и `dns.main.primary.domain` для AdGuard Home DoH).

> [!CAUTION]
> ### ⚠️ Критическое требование к Cloudflare (Только «Серое облако» / DNS-Only)
> Все DNS-записи доменов и поддоменов (`A`-записи для основного домена, поддомена DoH `dns.` и поддомена Steal `cdn.`) в панели управления Cloudflare **обязаны** находиться строго в режиме **DNS-Only (Серое облако)**!  
> Включение проксирования Cloudflare (Оранжевое облако) заблокирует L4 SNI-маршрутизацию, REALITY и работу xHTTP.
...
Мастер установки интерактивно запросит параметры развертывания (в скобках указаны значения по умолчанию, применяемые нажатием **Enter**):

| Шаг | Параметр | Значение по умолчанию | Описание |
| :--- | :--- | :--- | :--- |
| **—** | **Режим установки** | `2` (при наличии базы) | `1` — Clean Install (С нуля), `2` — Safe Migration (Обновление с сохранением базы и клиентов). |
...
> [!IMPORTANT]
> **Обязательное действие после завершения скрипта:**  
...
Скрипт формирует единую мультиформатную подписку **Client-Unified**, в которую автоматически упаковываются все сгенерированные протоколы. Все реквизиты доступа сохраняются в файл: **`/root/vpn_credentials.txt`**.
...
* **Clash / Mihomo YAML:** `https://yourdomain.online/my-post-key/ВАШ_SUB_ID?clash=1`  
...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: |
| **iOS / iPadOS** | **Happ Proxy** / **Streisand** / **FoXray** | ✔️ | ✔️ | ✔️ | ✔️ (v3.1) | ✔️ |
| **Android** | **v2rayNG** / **NekoBox** / **Sing-box** | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| **Windows** | **v2rayN** (v6.40+) / **NekoBox** / **Mihomo** | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| **macOS** | **V2RayXS** / **FoXray** / **NekoBox** | ✔️ | ✔️ | ✔️ | ✔️ (v3.1) | ✔️ |
| **Роутеры** | **Keenetic** / **OpenWrt (Podkop)** | ✔️ | ✔️ | ✔️ (клиент) | ✔️ (v2.0) | ✔️ |

---

## 🌐 Настройка роутеров для приватного AdGuard Home DoH

При установке AdGuard Home защищается уникальным идентификатором `ClientID` (по умолчанию `home-router`). Неавторизованные запросы ботов и сканеров отбрасываются с кодом `REFUSED`.
...
1. Перейдите в веб-интерфейс Keenetic (`192.168.1.1`) -> **«Сетевые правила»** -> **«Интернет-фильтр»** (или свойства подключения -> **«Серверы DNS»**).
2. Нажмите **«Добавить сервер DNS»**:
   * **Адрес DNS (Bootstrap):** `77.88.8.8` или `9.9.9.9` *(не вводите IP сервера: порт 53 закрыт фаерволом снаружи)*;
...
3. Нажмите **«Сохранить и применить»**. Все DNS-запросы клиентов локальной сети будут шифроваться и фильтроваться на Вашем VPS.
...
* **Certbot:** Системный таймер `certbot.timer` запускается дважды в сутки. При успешном продлении вызывается деплой-хук `/etc/letsencrypt/renewal-hooks/deploy/nginx-reload.sh`, который устанавливает права `755 / 644` для беспрепятственного чтения сертификатов демонами `nginx` и `nobody` (Xray), после чего безопасно перезагружает конфигурацию: `systemctl reload nginx`.
* **acme.sh:** Обновление сертификатов контролируется встроенным заданием `cron` с перезагрузкой Nginx.
...
Для исключения повреждения базы данных SQLite (`x-ui.db`) и WAL-журналов резервное копирование и восстановление осуществляются с кратковременной остановкой служб:
