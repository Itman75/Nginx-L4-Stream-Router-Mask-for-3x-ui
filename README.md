...
1. Создайте подключение типа **`vless`** со следующими параметрами:
   * **Порт (Port):** Внутренний порт xHTTP, заданный на Шаге 2 (по умолчанию `50443`) [1.1.2].
   * **IP для прослушивания (Listen IP):** `127.0.0.1` (изолирует порт внутри сервера) [1.1.2].
   * **Сеть (Network):** Выберите **`xhttp`** [1.1.2].
2. В расширенных параметрах транспорта задайте настройки:
   * **Путь (Path):** Секретный путь xHTTP, заданный на Шаге 2 (по умолчанию `/xhttp-stream`) [1.1.2].
   * **Хост (Host):** Ваш основной домен (например, `your.primary.domain`) [1.1.2].
   * **Режим (Mode):** Строго **`stream-one`** *(Обязательный параметр для корректной gRPC h2c-интеграции со шлюзом Nginx)* [1.1.2].
   * **Padding Bytes:** Диапазон рандомизации пакетов, например `100-1000` [1.1.2].
   * **Padding Obfs Mode:** Включен (`true`) [1.1.2].
3. **Безопасность (Security):** Установите значение **`none`** *(так как TLS-терминация трафика полностью обеспечивается веб-сервером Nginx на порту 443)* [1.1.2].
4. **Accept Proxy Protocol:** Выключен (`false`) [1.1.2].
