...
Проект предназначен для автоматизированного развёртывания на чистых операционных системах **Ubuntu (20.04+)** и **Debian (11+)**. Он объединяет скрипт усовершенствованной защиты ОС (`secure-vps.sh`) и официальный L4 Stream-маршрутизатор Nginx (`setup_mask.sh` v4.1.0) с поддержкой панели управления **3X-UI**.
...
Проект поддерживает **параллельное или раздельное использование** двух архитектурных сценариев REALITY на одном VPS:
...
* Все обычные HTTPS-запросы к этим доменам прозрачно перенаправляются Xray-ом на локальный Nginx (`127.0.0.1:9443`), где отдается валидный SSL-сертификат и сайт-маска.

### 2. Сценарий 2: Classic External REALITY (Внешний камуфляж)
* В качестве SNI используются любые известные внешние ресурсы (`swdist.microsoft.com`, `stream.is74.ru`, `www.apple.com` и др.).
...
* Все ваши собственные домены используются под маскировку Nginx, веб-панель 3X-UI, подписки и транспорт xHTTP.

---

## 🛡️ Комплексная защита системы

### Этап 1: Подготовка VPS и укрепление безопасности (`secure-vps.sh`)
...
### Этап 2: L4 Stream Router и маскировка (`setup_mask.sh` v4.1.0)
* **Мультиплексирование на портах TCP 443 и 8443:** Nginx Stream анализирует SNI-заголовки запросов (`ssl_preread`) без расшифровки TLS-потока:
  ...
  * Запросы к **собственным доменам Steal-Oneself** уходят на привязанные локальные порты (`reality_backend_45443`).
  * Запросы к **внешним SNI Classic REALITY** уходят на свои выделенные локальные порты (`reality_backend_47443`).
...
* **Мультидоменный SSL (Multi-Cert Let's Encrypt):** Выпуск независимых сертификатов для всех собственных доменов через Certbot с автонастройкой прав (`644`) и `deploy-hooks`.
...
> [!NOTE]
> Во вкладке инбаунда **«Поток» (Stream)** установите переключатель **«Реальный IP клиента»** в положение **`Выкл. / напрямую`**, а **Accept Proxy Protocol** включите (`true`).
...
* **Server Names (SNI):** Ваши собственные домены Steal-Oneself.
...
#### Вариант Б: Режим Classic External REALITY (Внешний SNI)
* **Порт (Port):** Выделенный локальный порт (например, `47443`).
* **IP для прослушивания (Listen IP):** `127.0.0.1`
* **Accept Proxy Protocol (xver):** `1 (Включить)` ⚠️
* **Безопасность (Security):** `Reality`
* **Цель (Target/Dest):** `swdist.microsoft.com:443` (или `подходящий в вашем регионе`)
* **Proxy Protocol для Dest:** `0 (Выключить)` ⚠️
* **Server Names (SNI):** `swdist.microsoft.com` (или соответствующий внешний SNI)
...
> [!WARNING]
> **ОБЯЗАТЕЛЬНАЯ НАСТРОЙКА КЛИЕНТСКИХ ПРИЛОЖЕНИЙ (Телефон / ПК):**
> Когда вы копируете VLESS-ссылку из панели 3X-UI, в ней по умолчанию содержится локальный адрес `127.0.0.1` и внутренний порт (например, `47443`).
> **В приложении (Nekoray, v2rayNG, Happ, Streisand) обязательно измените профиль:**
> * **Адрес (Address):** укажите **Внешний IP сервера** или ваш домен (например, `stat-isp.host74.ru`).
> * **Порт (Port):** укажите **`443`** (внешний порт Nginx Stream).
...
| `[Порты REALITY]/TCP` | Локальный | **Заблокирован** | Порты инбаундов REALITY (`45443`, `47443` и др., доступ через Nginx Stream) |
