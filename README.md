...
Проект развёртывается скриптами усиления безопасности ОС с интеграцией панели управления **3X-UI** (`secure-vps.sh`) и официальный L4 Stream-маршрутизатор Nginx (`setup_mask.sh` v4.1.0), защитой от активного сетевого сканирования (Active Probing) со стороны систем DPI и полной поддержкой транспортов VLESS REALITY, xHTTP и Hysteria 2.
