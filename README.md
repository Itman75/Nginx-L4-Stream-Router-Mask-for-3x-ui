...
* **Тюнинг сетевого стека (Proxy Tuning):** Независимо от выбранных конфигураций, запуск первого скрипта (1й Шаг) на начальном этапе оптимизирует параметры ядра через `sysctl`. Для предотвращения потерь пакетов при пиковых нагрузках увеличиваются лимиты сетевых очередей (`somaxconn`, `backlog`), расширяются TCP-буферы (`rmem`, `wmem`), а также деактивируется механизм TCP Slow Start после простоя.
...
* **Строгая конфигурация брандмауэра (UFW):** По умолчанию блокируются все входящие соединения. Доступ открывается только к строго необходимым портам:
  ...
  * **TCP:** `80`, `443`, `8443`, (`10443` - открывается по умолчанию для web-панели 3x-ui).
  * **UDP:** `443`, `8443`.
...
### Уустановка и первоначальная настройка панели 3X-UI
...
### 2. Интерактивное развёртывание маршрутизации трафика (Nginx L4 Stream)
