...
#
# ==============================================================================
# Production AutoSetup: Hardened Master Engine v7.1.1 Universal (All-in-One)
...
echo -e "${CYAN}=====================================================================${NC}"
echo -e "${GREEN}  All-in-One Master Installer v7.1.1 Universal (Stable Edition)      ${NC}"
...
echo -e "  1) ${GREEN}${BOLD}Express (Автопилот — Рекомендуется)${NC} — Развёртывание за 3 вопроса."
echo -e "  2) ${YELLOW}${BOLD}Custom (Эксперт)${NC} — Ручной контроль над каждым компонентом."
...
if [ "$INSTALL_MODE" = "1" ]; then
    ...
    prompt_yes_no "Включить Steal-Oneself REALITY?" "y" && ENABLE_STEAL=1 || ENABLE_STEAL=0
    ...
    echo -e "${YELLOW}Выбор маскировочного сайта:${NC}"
    echo -e "  1) ${GREEN}DataSphere Analytics Enterprise${NC}"
    echo -e "  2) ${GREEN}CosmosCloud NextGen${NC}"
    echo -e "  3) Стандартная заглушка Nginx"
    prompt_default "Ваш выбор" "1" DECOY_MODE

    prompt_default "Метод SSL (1-Certbot, 2-acme.sh)" "1" SSL_ENGINE_CHOICE
    if [ "$SSL_ENGINE_CHOICE" = "2" ]; then
        prompt_default "Cloudflare Auth (1-Token, 2-Global Key)" "1" CF_AUTH_METHOD
...
# ----------------------- Этап 2: Тюнинг ядра -----------------------
echo
log "Этап 2: Настройка ядра BBR, fq и сокетов..."
...
EOF
ok "Ядро и лимиты оптимизированы."

# ----------------------- Этап 3: SSH и Fail2ban -----------------------
echo
log "Этап 3: Защита SSH и настройка Fail2ban..."
...
if /usr/sbin/sshd -t; then
    ...
    ok "SSH настроен на порту $TARGET_SSH_PORT."
else
...
# ----------------------- Этап 5: Nginx & SSL -----------------------
echo
log "Этап 5: Подключение Nginx Mainline и выпуск SSL..."
...
EOF
nginx -t || die "Ошибка конфигурации ACME сервера."
...
EOF
    ...
    ok "AdGuard Home DoH активен на порту 53."
fi

# ----------------------- Этап 7: Веб-маска -----------------------
echo
log "Этап 7: Генерация веб-маскировки (Decoy Front)..."
...
# ----------------------- Этап 9: 3X-UI SQLite -----------------------
echo
log "Этап 9: Синхронизация инбаундов и Единого клиента в 3X-UI (Zero-Touch)..."
...
# ----------------------- Этап 10: Фаервол UFW & Port Hopping -----------------------
echo
...
ok "Фаервол UFW настроен."

# ----------------------- Финал -----------------------
CRED_FILE="/root/vpn_credentials.txt"
cat << EOF > "$CRED_FILE"
=====================================================================
  УЧЕТНЫЕ ДАННЫЕ ВАШЕГО СЕРВЕРА (All-in-One v7.1.1 Stable Edition)
...
echo
echo -e "${GREEN}=====================================================================${NC}"
echo -e "${GREEN}  СИСТЕМА УСПЕШНО РАЗВЕРНУТА И ГОТОВА К РАБОТЕ (v7.1.1 STABLE)!      ${NC}"
echo -e "${GREEN}=====================================================================${NC}"
...
exit 0        die "Параметр $name ('$val') содержит недопустимые символы. Используйте латиницу, цифры, дефис и слэши."
    fi
}
